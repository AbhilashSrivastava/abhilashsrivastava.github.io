...
XPRSHelper.presetTypes = {
		...
		"HTML":{"TYPE":"HTML","NAME":"HTML","GROUP":"ELEMENTS","COLOR":"#999999","PAGE":2},
...
XPRSHelper.getShoprocketUrl = function(){
	...
	if (dashboardUrl){
		...
		return "http://dashboard.shoprocket.co";
	}
};
		...
		if (XPRSHelper.currentUser["type"] == "GUEST" || nextUrl != "" || forceDialog){
			...
			if (typeof LoginModule == "undefined"){
			 ...
			 login_css.attr({ 
			      rel:  "stylesheet",
			      type: "text/css",
			      href: XPRSHelper.getServerPath() + "/css/login.css?v=140"
			    });
			 $("head").append(login_css);
			 
			 
			 $.ajax({
				  url: XPRSHelper.getServerPath() + "/js/login.js?v=140",
				  dataType: 'script',
				  success: function(){
					  setTimeout(function(){
